...
def EditorView(request,LOGIN_ID,id,version,role):
	...
	for i in doc:
		flag = i.lock
	if(flag==0 and role=='Contributor'):
		print("here hehehehe")
...
def login(request):
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			return render(request, 'mainpage.html', {"error":"Do not leave fields empty. Login again"})
	...
	if request.method == 'POST':
		...
		if choice == "collaborator" :
			...
			try:
				...
				return render(request, 'createnew.html', {'LOGIN_ID':LOGIN_ID,"error":"Wrong reviwer User ID"})
		else:
			index=4
			reviewer=LOGIN_ID
		...
		try:
			...
			collablist = list(set(collablist))
			for i in collablist:
					print("3")
...
def signUp(request):
	if request.method == 'POST':
		...
		elements={"Username":LOGIN_ID,"Password":PASSWORD,"Email":email,"Name":name,"Institution":institution,"Profession":profession}
		if form.is_valid():
			print(7)
			...
			for i in elements.keys():
				if elements[i]=="":
					return render(request, 'mainpage.html', {"error":"Cannot leave "+i+" field empty .Sign in again."})
